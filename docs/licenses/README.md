...
This application builds `app/libs/libbox.aar` from the `third_party/sing-box` submodule at commit `4626aa2cb07db5a453f689cf348ba9d327e07820` (`v1.12.11`). The generated AAR is intentionally excluded from Git history.
...
To reproduce the Android library, initialize the submodule and run `powershell -ExecutionPolicy Bypass -File .\tools\build-libbox.ps1` with Android SDK, NDK, Go, and OpenJDK 17 installed. Gradle runs this script automatically before a build when the AAR is absent.
